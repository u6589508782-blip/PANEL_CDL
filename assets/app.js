...
function normalizeHash(){
  // Convierte: #inventario -> #/inventario
...
function openMenu(){
  ...
  if(!b || !off) return;
  try{
    b.Offcanvas.getOrCreateInstance(off).show();
  }catch(_e){}
}
...
function logout(){
  ...
  if(location.hash && location.hash !== '#/planificacion'){
...
function wireTopbar(){
  ...
  if(btnOpenLogin){
    btnOpenLogin.addEventListener('click', (e)=>{
      e.preventDefault();
      // Si no hay sesión, abre modal.
      if(!state.token) openLoginModal();
      // Si hay sesión, el dropdown de Bootstrap ya hace su trabajo.
    });
  }
...
function wireLoginModal(){
  ...
  const doLogin = async ()=>{
    ...
    try{
      const out = await apiPost({ path:'login', user, pass });
...
function buildMenu(){
  ...
  if(isUL){
    // Compatible con: <ul class="list-group list-group-flush" id="menuItems">
    ...
    menuTarget.innerHTML = items.map(
      ([k,l])=>`<a href="#/${k}" class="menu-item">${escapeHtml(l)}</a>`
    ).join('');
  }
}
